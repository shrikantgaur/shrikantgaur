# 👋 Hi, I'm Shri Kant Gaur 😄

Hi there! I'm Shri Kant Gaur, a passionate software developer with 5+ years of experience in the world of coding. I'm based in Chandigarh and love to create digital solutions that make a difference.

## 💼 What I Do

- **Front-End**: HTML, CSS, Bootstrap, JavaScript, jQuery, React.js
- **Back-End**: PHP (Symfony), Node.js, MySQL
- **CMS & Open Source**: WordPress (Plugin & Theme Development), Shopify, and Prestashop

I'm dedicated to delivering exceptional user experiences and crafting efficient server-side solutions. Feel free to explore my repositories and discover the projects I've been working on.

## 🚀 My Projects

Here are some of the projects I'm proud of:

- 🌟 **[Shrikant Social Post Builder](https://github.com/shrikantgaur/shrikant-social-post-builder)** — A professional WordPress plugin to compile and format multiple WordPress posts into ready-to-share social media messages in one click. ([Official WordPress.org Plugin Page](https://wordpress.org/plugins/shrikant-social-post-builder/))
- 📱 **[RoamOne](https://github.com/shrikantgaur/roamone)**
- 💻 **[React Node CRUD App](https://github.com/shrikantgaur/react-node-crud-app)**

Check them out and let me know what you think!

## 📫 Let's Connect

I'm always open to collaboration and new opportunities. Don't hesitate to reach out:

- **WordPress.org Profile**: [![WordPress](https://img.shields.io/badge/WordPress.org-Profile-21759B)](https://profiles.wordpress.org/shrikantgaur)
- **LinkedIn**: [![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue)](https://www.linkedin.com/in/shri-kant-gaur-717476171)
- **GitHub**: [![GitHub](https://img.shields.io/badge/GitHub-Follow-brightgreen)](https://github.com/shrikantgaur/)

Let's create amazing things together!
